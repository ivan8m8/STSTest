...
Отказ от ``toString`` обусловлен тем, что программист может забыть переопределить эту функцию, в то время как о 
необходимости переопределения метода ``inline()`` программисту напомнит компилятор.
